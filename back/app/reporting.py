def generate_report(audio_path, results):
    ...
    pdf.cell(0, 7, f"Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True)
    ...
    pdf.cell(0, 7, f"Average Deepfake Score: {json_report['average_deepfake_score']}", ln=True)
    pdf.cell(0, 7, f"Average Genuine Score: {json_report['average_genuine_score']}", ln=True)  # Added
    ...
    pdf.cell(0, 8, "This is an official forensic report generated by Prison Voice Forensics Bureau.", ln=True, align="C")
    pdf.cell(0, 8, "For internal use only. Contact bureau for verification.", ln=True, align="C")
